...
import json
from src.talking_detector.ASD import ASD as L_ASD
from src.talking_detector_lrasd.ASD import ASD as LR_ASD

import argparse


def process_video(ASD_MODEL, video_path, output_dir=None, frame_offset=0):
    """
...
def main():
    ...
    parser.add_argument('--model', type=str, default=None, help='ASD model file')
    opt = parser.parse_args()

    # ==================== LOAD MODEL ====================
    if opt.model == "finetuning_TalkSet.model": 
        ASD_MODEL = L_ASD()
        ASD_MODEL.loadParameters("model-bin/finetuning_TalkSet.model")
        ASD_MODEL = ASD_MODEL.cuda().eval()
        print("Light-ASD-Model loaded successfully.")
    elif opt.model == "finetuning_TalkSet_lrasd.model": 
        ASD_MODEL = LR_ASD()
        ASD_MODEL.loadParameters("model-bin/finetuning_TalkSet_lrasd.model")
        ASD_MODEL = ASD_MODEL.cuda().eval()
        print("Light-Robust-ASD-Model loaded successfully.")
    elif opt.model == "finetuning_MCoRec_5_epoch.model": 
        ASD_MODEL = LR_ASD()
        ASD_MODEL.loadParameters("model-bin/finetuning_MCoRec_5_epoch.model")
        ASD_MODEL = ASD_MODEL.cuda().eval()
        print("MCoRec-ASD-Model loaded successfully.")
    else:
        print("Please choose a model for ASD.")
    ...
    output_path = process_video(ASD_MODEL, opt.video, opt.output_dir, frame_offset)

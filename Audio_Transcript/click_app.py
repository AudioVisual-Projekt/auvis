#%%
import click
from models.whisper import whisper_inference_with_segments_df
from models.pyannote import get_pyannote_access_token, pyannote_inference_df
from models.conversation_transcription import combine_consecutive_speakers, \
...
@click.command()
@click.option('--wav_fname', type=click.Path(exists=True), prompt='Please provide path to WAV file.')
@click.option('--model_type', type=click.Choice(choices=WHISPER_MODELS), default='base', 
              prompt='Please provide whisper model.', help='Defines the model type in whisper. The *.en models are English only models.')
...
@click.command()
@click.option('--url', type=click.STRING, prompt='Please provide the url to an mp3 file.')
@click.option('--model_type', type=click.Choice(choices=WHISPER_MODELS), default='base', 
              prompt='Please provide whisper model.', help='Defines the model type in whisper.')
